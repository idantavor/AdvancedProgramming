...
bool BattleManager::runBattle(const string & dirPath)
{
	...
	// set player boards
	playerA->setBoard(boardAsCharMat, this->boardSize, this->boardSize); // should only contain each players board but since it is almost empty imple doesn't matter
	playerB->setBoard(boardAsCharMat, this->boardSize, this->boardSize);
