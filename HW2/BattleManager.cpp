...
bool BattleManager::runBattle(const string & dirPath)
{
	...
	for (int i = 0; i < this->boardSize; i++) {
		boardAsCharMat[i] = this->board[i].c_str();
	}
	try {
	...
	// set player boards
	//TODO 
	playerA->setBoard(0,boardAsCharMat, this->boardSize, this->boardSize); // should only contain each players board but since it is almost empty imple doesn't matter
	playerB->setBoard(1,boardAsCharMat, this->boardSize, this->boardSize);
